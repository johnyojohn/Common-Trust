...
import {
  deleteUserFromClass, deleteClassFromUser,
  deleteAllCommentsFromPost, deletePostFromUser,
} from "./deleteUtils.js";
...
router.put("/:id", async (req, res) => {
  ...
  const departmentAbbr = "departmentAbbr" in req.body ?
    req.body.departmentAbbr : null;
  const courseNumber = "courseNumber" in req.body ?
    req.body.courseNumber : null;
  const courseFullTitle = "courseFullTitle" in req.body ?
    req.body.courseFullTitle : null;
  const instructorsIdArr = "instructorsIdArr" in req.body ?
    req.body.instructorsIdArr : null;
  ...
  try {
    ...
    if (!classDoc.exists()) {
      ...
      const newInstructors = instructorsIdArr ?
      instructorsIdArr : originalInstructors;
      ...
      if (instructorsIdArr !== null) {
        const addedInstructors = newInstructors.filter(
            (id) => !originalInstructors.includes(id));
        const removedInstructors = originalInstructors.filter(
            (id) => !newInstructors.includes(id));
